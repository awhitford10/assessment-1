...
def optimal_change(cost,payment):
    ...
    if not((type(cost)==float or type(cost)==int) and (type(payment)==float or type(payment)==int)):
    ...
    elif payment == cost:                                                       #catch exact change
        ...
        output = [
            f"The optimal change for an item that costs $"+'{:.2f}'.format(cost)+
            ' with an amount paid of $'+'{:.2f}'.format(payment)+" is "]        #beggining of output
